...
# Function to update the object's position based on finger count
def update_object_position(count):
    global obj_x, obj_y
    step_size = 20  
    
    if count == 1:
        obj_x += step_size  # Move right
    elif count == 2:
        obj_x -= step_size  # Move left
    elif count == 3:
        obj_y -= step_size  # Move up
    elif count == 4:
        obj_y += step_size  # Move down
...
# Main loop for processing video and hand landmarks
def main_loop():
    ...
    label_video.img = img  # Keep a reference!
...
root.geometry("700x600")  # Set a fixed size for the window
...
# Create a movable object (a square)
obj_size = 20
obj_x, obj_y = 0, 0  # Starting position
